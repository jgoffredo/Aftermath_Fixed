...
--Maikerumine added hackish follow code
	on_rightclick = function (self, clicker)
		mobs:face_pos(self,clicker:get_pos())
		mobs:team_player(self,clicker:get_pos())
		if self.state ~= "path" and self.state ~= "following" then
		local_chat(clicker:get_pos(),"Sam: Let's go kick some Mob butt!",3)
		...
		local_chat(clicker:get_pos(),"Bajan: Let's go kick some Mob butt!",3)
			...
			if self.owner == "" then
				...
				minetest.register_on_player_receive_fields(function(clicker, formname, fields)
					...
					if fields.gsethome then
						self.floats = self.object:get_pos()
					end
...
--Maikerumine added hackish follow code
	on_rightclick = function (self, clicker)
		...
		mobs:team_player(self,clicker:get_pos())
		if self.state ~= "path" and self.state ~= "following" then
		local_chat(clicker:get_pos(),"John: Let's go grief some monsters!",3)
			if not self.tamed then
				self.tamed = true
				self.follow = true
			end
...
--Maikerumine added hackish follow code
	on_rightclick = function (self, clicker)
		...
		mobs:team_player(self,clicker:get_pos())
		if self.state ~= "path" and self.state ~= "following" then
		local_chat(clicker:get_pos(),"Jannette: Stop flirting with me!",3)
			if not self.tamed then
				self.tamed = true
				self.follow = true
			end
		...
		on_rightclick = function(self, clicker)
		local item = clicker:get_wielded_item()
		local_chat(clicker:get_pos(),"Krock: Stop flirting with me!",3)
		if item:get_name() == "mobs_fallout:meat" or item:get_name() == "farming:bread" then
			local hp = self.object:get_hp()
			if hp + 4 > self.hp_max then return end
			if not minetest.settings:get_bool("creative_mode") then
...
--Maikerumine added hackish follow code
	on_rightclick = function (self, clicker)
		...
		mobs:team_player(self,clicker:get_pos())
		if self.state ~= "path" and self.state ~= "following" then
		local_chat(clicker:get_pos(),"Crybaby: I am too whimpy to fight mobs, but I can do my best!",3)
			if not self.tamed then
				self.tamed = true
				self.follow = true
			end
		...
		on_rightclick = function(self, clicker)
		local item = clicker:get_wielded_item()
		local_chat(clicker:get_pos(),"Just_Test_Griefer: I am too whimpy to fight mobs, but I can do my best!",3)
		if item:get_name() == "mobs_fallout:meat" or item:get_name() == "farming:bread" then
			local hp = self.object:get_hp()
			if hp + 4 > self.hp_max then return end
			if not minetest.settings:get_bool("creative_mode") then
...
--Maikerumine added hackish follow code
	on_rightclick = function (self, clicker)
		...
		mobs:team_player(self,clicker:get_pos())
		if self.state ~= "path" and self.state ~= "following" then
		local_chat(clicker:get_pos(),"Sepia Sam: MESE sword + Monster = My pleasure!",3)
			if not self.tamed then
				self.tamed = true
				self.follow = true
			end
...
--Maikerumine added hackish follow code
	on_rightclick = function (self, clicker)
		...
		mobs:team_player(self,clicker:get_pos())
		if self.state ~= "path" and self.state ~= "following" then
		local_chat(clicker:get_pos(),"O.G. Sam: Mobs, let me at 'em, I'll splat 'em!!!",3)
			if not self.tamed then
				self.tamed = true
				self.follow = true
			end
		...
		on_rightclick = function(self, clicker)
		local item = clicker:get_wielded_item()
		local_chat(clicker:get_pos(),"Hobo: Mobs, let me at 'em, I'll splat 'em!!!",3)
		if item:get_name() == "mobs_fallout:meat" or item:get_name() == "farming:bread" then
			local hp = self.object:get_hp()
			if hp + 4 > self.hp_max then return end
			if not minetest.settings:get_bool("creative_mode") then
...
--Maikerumine added hackish follow code
	on_rightclick = function (self, clicker)
		...
		mobs:team_player(self,clicker:get_pos())
		if self.state ~= "path" and self.state ~= "following" then
		local_chat(clicker:get_pos(),"Vanessa: I'll code out the very instance of those mobs!",3)
			if not self.tamed then
				self.tamed = true
				self.follow = true
			end
		...
		on_rightclick = function(self, clicker)
		local item = clicker:get_wielded_item()
		local_chat(clicker:get_pos(),"Simon: I'll code out the very instance of those mobs!",3)
		if item:get_name() == "mobs_fallout:meat" or item:get_name() == "farming:bread" then
			local hp = self.object:get_hp()
			if hp + 4 > self.hp_max then return end
			if not minetest.settings:get_bool("creative_mode") then
...
--Maikerumine added hackish follow code
	on_rightclick = function (self, clicker)
		...
		mobs:team_player(self,clicker:get_pos())
		if self.state ~= "path" and self.state ~= "following" then
		local_chat(clicker:get_pos(),"Female Sam: Minetest is the greatest voxel game ever created!",3)
			if not self.tamed then
				self.tamed = true
				self.follow = true
			end
		...
		on_rightclick = function(self, clicker)
		local item = clicker:get_wielded_item()
		local_chat(clicker:get_pos(),"Infantry_man: Minetest is the greatest voxel game ever created!",3)
		if item:get_name() == "mobs_fallout:meat" or item:get_name() == "farming:bread" then
			local hp = self.object:get_hp()
			if hp + 4 > self.hp_max then return end
			if not minetest.settings:get_bool("creative_mode") then
...
--Maikerumine added hackish follow code
	on_rightclick = function (self, clicker)
		...
		mobs:team_player(self,clicker:get_pos())
		if self.state ~= "path" and self.state ~= "following" then
		local_chat(clicker:get_pos(),"Battlefield 3 Soldier: All suited up, let's roll out and destroy those creatures!",3)
			if not self.tamed then
				self.tamed = true
				self.follow = true
			end
		...
		on_rightclick = function(self, clicker)
		local item = clicker:get_wielded_item()
		local_chat(clicker:get_pos(),"Mage: All suited up, let's roll out and destroy those creatures!",3)
		if item:get_name() == "mobs_fallout:meat" or item:get_name() == "farming:bread" then
			local hp = self.object:get_hp()
			if hp + 4 > self.hp_max then return end
			if not minetest.settings:get_bool("creative_mode") then
