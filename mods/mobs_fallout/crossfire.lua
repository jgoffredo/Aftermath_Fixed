...
--Maikerumine added hackish follow code
	on_rightclick = function (self, clicker)
		...
		local_chat(clicker:get_pos(),"Mr. White: Let's go kick some Mob butt!",3)
...
--[[
	on_rightclick = function (self, clicker)
		...
		mobs:team_player(self,clicker:get_pos())
		if self.state ~= "path" and self.state ~= "following" then
			local_chat(clicker:get_pos(),"Mr. Black: Grrrrrrrrrrrr!",3)
				if not self.tamed then
					self.tamed = true
					self.follow = true
			end
...
--TENPLUS1 and CProgrammerRU AWESOME CODES.
	...
	on_rightclick = function(self, clicker)
		local item = clicker:get_wielded_item()
		local_chat(clicker:get_pos(),"Mr. Pink: My name is Norman, how may I assist?",3)
		if item:get_name() == "mobs_fallout:meat" or item:get_name() == "farming:bread" then
			local hp = self.object:get_hp()
			if hp + 4 > self.hp_max then return end
			if not minetest.settings:get_bool("creative_mode") then
