...
set_velocity = function(self, v)
	...
	local partial_yaw = self.object:get_yaw()
	local yaw = 0
	local partial_vel = self.object:get_velocity()
	local vel = 0

	if partial_yaw ~= nil then
		yaw = partial_yaw + self.rotate
	end

	if partial_vel ~= nil then
		vel = partial_vel.y
	end
...
local npc_attack = function(self)
	...
	-- JGoffredo: Is very strange this fail.
	local s = self.object:get_pos()
	...
	if s ~= nil then
		...
		for n = 1, #objs do
			...
			and obj.type == "monster" then
				...
				local p = obj.object:get_pos()  --maikerumine added local
...
local do_states = function(self, dtime)
	...
	if self.state == "stand" then
		...
		if random(1, 4) == 1 then
			...
			if s ~= nil and self.type == "npc" then
	...
	elseif self.state == "walk" then
		...
		and self.jump
		and get_velocity(self) <= 0.5 then
